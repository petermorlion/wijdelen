...
namespace WijDelen.Reports.Queries {
    public class RequestsDetailsQuery : IRequestsDetailsQuery {
        ...
        public IEnumerable<RequestsDetailsViewModel> GetResults(int groupId, DateTime startDate, DateTime stopDate) {
            ...
            var requestsQuery = session.CreateSQLQuery("SELECT r.AggregateId, r.CreatedDateTime, u.Email, r.Description, COUNT(y.Response) AS 'YesCount', COUNT(n.Response) AS 'NoCount', COUNT(nn.Response) AS 'NotNowCount' " +
                                                       ...
                                                       "WHERE r.CreatedDateTime >= :startDate AND r.CreatedDateTime <= :stopDate AND r.GroupId = :groupId " +
                                                       "GROUP BY r.AggregateId, r.CreatedDateTime, u.Email, r.Description, y.Response, n.Response, nn.Response " +
